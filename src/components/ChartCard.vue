<script setup lang="ts">
    ...
    const newExpenses = expenses.map(expense => ({category: expense.expenseCategory, amount: expense.expenseAmount}))
    ...
    const categoryTotalAmountArray: [string, number][] = Object.entries(categoryTotalAmount)
    ...
    const sortedNumbers = Object.values(sortedData).filter(value => typeof value === 'number')
 
    const data = {
        labels: lst,
        datasets: [
            {
                backgroundColor: ['#F66D44', '#2D87BB', '#E6F69D', '#AADEA7', '#FEAE65'],
                data: sortedNumbers
            }
        ],
        chartOptions: {
...
<template>
    <div class="main-container">
        <div class="pie-chart"><Pie :data="data" :height="300" :width="400"/></div>
    </div>
</template>
